...
export default class NPC extends Phaser.GameObjects.Sprite {
    ...
    constructor(name: string, scene: Phaser.Scene, x: number, y: number, texture: string, frame?: string | integer) {
    ...
    private shouldStartAnimation(newAnimKey: string): boolean {
        ...
        const isNewAnimation: boolean =
            Boolean(newAnimKey) && this.anims.currentAnim && this.anims.currentAnim.key !== newAnimKey;
    ...
    private wander(): void {
        if (this.lastWanderAction instanceof NPCActionWait) {
            const distance =
                ...
                Math.round(
                    Math.random() * (this.MAX_WANDER_DISTANCE - this.MIN_WANDER_DISTANCE) + this.MIN_WANDER_DISTANCE
                );
            const xOrY = Boolean(Math.round(Math.random()));
            this.currentAction = new NPCActionTravel(
                this,
                {
            ...
            this.currentAction = new NPCActionWait(
                this,
                Math.round(
                    Math.random() * (this.MAX_WANDER_WAIT_SECONDS - this.MIN_WANDER_WAIT_SECONDS) +
                        this.MIN_WANDER_WAIT_SECONDS
                )
            );
        }
